...
HttpServer::HttpServer (const std::string &host, std::uint16_t port)
    : host_ (host), port_ (port), sock_fd_ (0), running_ (false),
      worker_epoll_fd_ (),
...
HttpServer::Start ()
{
  ...
  if (bind (sock_fd_, (sockaddr *)&server_address, sizeof (server_address))
      < 0)
    {
...
void
HttpServer::ProcessEvents (int worker_id)
{
  EventData *data;
  ...
  while (running_)
    {
      ...
      int nfds
          = epoll_wait (worker_epoll_fd_[worker_id], worker_events_[worker_id],
                        HttpServer::kMaxEvents, 0);
      if (nfds <= 0)
        {
          active = false;
          continue;
...
void
HttpServer::HandleEpollEvent (int epoll_fd, EventData *data,
                              std::uint32_t events)
{
  int fd = data->fd;
  ...
  if (events == EPOLLIN)
    {
      ...
      if (byte_count > 0)
        { // we have fully received the message
          ...
          control_epoll_event (epoll_fd, EPOLL_CTL_MOD, fd, EPOLLOUT,
                               response);
          delete request;
        }
      ...
      else
        {
          if (errno == EAGAIN || errno == EWOULDBLOCK)
            { // retry
              request->fd = fd;
              control_epoll_event (epoll_fd, EPOLL_CTL_MOD, fd, EPOLLIN,
                                   request);
            }
          else
            { // other error
    ...
    {
      response = data;
      ssize_t byte_count = send (fd, response->buffer + response->cursor,
                                 response->length, 0);
      if (byte_count >= 0)
        {
      ...
      else
        {
          if (errno == EAGAIN || errno == EWOULDBLOCK)
            { // retry
              control_epoll_event (epoll_fd, EPOLL_CTL_ADD, fd, EPOLLOUT,
                                   response);
            }
          else
            { // other error
...
void
HttpServer::HandleHttpData (const EventData &raw_request,
                            EventData *raw_response)
{
  ...
  // null-terminate the buffer if response cannot fit inside buffer
    ...
    {
      memcpy (raw_response->buffer, response_string.c_str (), kMaxBufferSize - 1);
...
HttpResponse
HttpServer::HandleHttpRequest (const HttpRequest &request)
{
...
void
HttpServer::control_epoll_event (int epoll_fd, int op, int fd,
