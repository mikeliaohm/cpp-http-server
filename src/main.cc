...
void
ensure_enough_resource (int resource, std::uint32_t soft_limit,
  ...
  if (setrlimit (resource, &new_limit))
    {
      ...
      std::cerr << "Consider setting the limit manually with ulimit"
                << std::endl;
      exit (-1);
    }
}
...
main (void)
{
  ...
  server.RegisterHttpRequestHandler ("/hello.html", HttpMethod::HEAD,
                                     send_html);
  ...
  try
    {
      ...
      // std::cout << "Setting new limits for number of threads.." <<
      // std::endl; ensure_enough_resource(RLIMIT_NPROC, 60000, 60000);
